...
import com.click_labs.click_labsworkflow.util.customview.ProgressDialog;
...
import static com.click_labs.click_labsworkflow.constants.AppConstant.SESSION_EXPIRED;
import static com.click_labs.click_labsworkflow.retrofit.ErrorUtils.DEFAULT_STATUS_CODE;
...
//        }
    ...
    public Boolean checkAuthorizationError(final APIError apiError) {
        if (apiError.getStatusCode() == SESSION_EXPIRED && apiError.getMessage().equals("Session has expired.")) {
            Util.restartAppOnSessionExpired(weakContext.get());
            return false;
        } else if (apiError.getStatusCode() == SESSION_EXPIRED) {
            new CustomAlertDialog.Builder(weakContext.get())
                    .setMessage(apiError.getMessage())
                    .setCancelable(false)
